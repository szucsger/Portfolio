...
let savingRateValue = document.querySelector("#saving-rate-value");
let transactionsList = document.querySelector("#tx-list");
let transactionState = document.querySelector("#empty-state");
let typeInput = document.querySelector("#tx-type");
let categoryInput = document.querySelector("#tx-category");
let amountInput = document.querySelector("#tx-amount");
let dateInput = document.querySelector("#tx-date");
let noteInput = document.querySelector("#tx-note");
let submitForm = document.querySelector("#tx-form");
let filterSelect = document.querySelector("#tx-filter");
let sortSelect = document.querySelector("#tx-sort");
let expenseChartCanvas = document.querySelector("#expense-category-canvas");
let incomeExpenseChartCanvas = document.querySelector("#income-expense-canvas");
...
function calculateSavingRate() {
    ...
    let totalExpenseValue = totalExpense();
...
function loadTransactions() {
    ...
    function normalizeTransaction(item, index) {
        ...
        if (tx.type !== "income" && tx.type !== "expense")
            return null;
        const category = typeof tx.category === "string" && tx.category.trim().length > 0
            ? tx.category.trim()
            : "Uncategorized";
        const amount = Number(tx.amount);
        if (!Number.isFinite(amount) || amount <= 0)
            return null;
        const date = typeof tx.date === "string" && tx.date.trim().length >= 7
            ? tx.date
            : fallbackDate;
        const id = Number(tx.id);
        const safeId = Number.isFinite(id) && id > 0 ? id : index + 1;
        const normalized = {
            id: safeId,
            type: tx.type,
            category,
...
function renderTransactions() {
    ...
    for (let i = 0; i < visibleTransactions.length; i++) {
        ...
        removeBtn.classList.add("btn", "btn-sm", "btn-outline-danger", "remove-btn");
...
submitForm.addEventListener("submit", (event) => {
    ...
    let inputCategory = categoryInput.value.trim();
    let inputAmount = parseFloat(amountInput.value);
    let inputDate = dateInput.value;
    let inputNote = noteInput.value.trim();
    if (!inputType ||
        !inputCategory ||
        isNaN(inputAmount) ||
        inputAmount <= 0 ||
...
function incomeVsExpenseChart() {
    let monthlyNetByMonth = {};
    for (let i = 0; i < transactions.length; i++) {
        let tx = transactions[i];
        if (!tx || !tx.date)
            continue;
        let monthKey = tx.date.slice(0, 7);
    ...
    let labels = Object.keys(monthlyNetByMonth).sort((a, b) => a.localeCompare(b));
    let runningBalance = 0;
    let balanceData = labels.map((label) => {
...
function getVisibleTransactions() {
    ...
    if (currentFilter !== "all") {
        visibleTransactions = visibleTransactions.filter((tx) => tx.type === currentFilter);
    }
